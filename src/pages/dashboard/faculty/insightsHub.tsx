...
export default function FacultyInsightsHubPage() {
    ...
    const [damageSeverity, setDamageSeverity] =
        React.useState<DamageSeverity>("minor");
    ...
    const loadAll = React.useCallback(async () => {
        ...
        try {
            ...
            const msg =
                err?.message || "Failed to load insights data. Please try again later.";
            setError(msg);
    ...
    function handleDamageFilesChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        <DashboardLayout title="Insights Hub">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h2 className="text-lg font-semibold leading-tight">Insights Hub</h2>
                        <p className="text-xs text-white/70">
                            Share feedback on books and report damaged copies to help keep the
                            library collection in good shape.
                        </p>
                    </div>
                </div>
            ...
            {error && !loading && <div className="mb-4 text-sm text-red-300">{error}</div>}
            ...
            <Card className="mb-6 bg-slate-800/60 border-white/10">
                <CardHeader>
                    ...
                    <p className="mt-1 text-xs text-white/70">
                        Rate books you&apos;ve read and add an optional comment.
                    </p>
                </CardHeader>
                    ...
                    <div className="grid gap-6 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1.2fr)]">
                        ...
                        <form className="space-y-4" onSubmit={handleSubmitFeedback}>
                            <div className="space-y-1.5">
                                ...
                                <Select value={feedbackBookId} onValueChange={(v) => setFeedbackBookId(v)}>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-white/80">Rating</label>
                                <div className="flex items-center gap-2">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between gap-2">
                                <h3 className="text-sm font-semibold text-white/90">Your feedback</h3>
                                ...
                                <div className="space-y-2 max-h-72 overflow-y-auto pr-1 support-scroll">
                                    {myFeedbacks.map((fb) => (
                                        ...
                                        >
                                            ...
                                            {fb.comment && (
                                                <p className="mt-1 text-[11px] text-white/80">{fb.comment}</p>
                                            )}
                                        </div>
                                    ))}
                                </div>
            ...
            <Card className="bg-slate-800/60 border-white/10">
                <CardHeader>
                    ...
                    <p className="mt-1 text-xs text-white/70">
                        If you notice a damaged book, submit a report with details and up to three
                        photos so the librarian can assess it.
                    </p>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div className="grid gap-6 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1.2fr)]">
                        ...
                        <form className="space-y-4" onSubmit={handleSubmitDamageReport}>
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-white/80">Select book</label>
                                <Select value={damageBookId} onValueChange={(v) => setDamageBookId(v)}>
                                    <SelectTrigger className="h-9 w-full bg-slate-900/70 border-white/20 text-white">
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-white/80">
                                    Extra notes <span className="text-white/40">(optional)</span>
                                </label>
                                <textarea
                                    value={damageNotes}
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-white/80 flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] text-white/50">(max 3 images)</span>
                                </label>
                                <Input
                                    type="file"
                                    multiple
                            ...
                            <p className="text-[10px] text-amber-200/80 flex items-start gap-1.5">
                                ...
                                A librarian will review your report and may assess a fee if the damage is
                                serious or due to misuse.
                            </p>
                        </form>
                        ...
                        <div className="space-y-3">
                            ...
                            {loading ? (
                                ...
                                <div className="rounded-md border border-dashed border-white/20 px-3 py-4 text-xs text-white/60">
                                    ...
                                    <br />
                                    <span className="opacity-80">
                                        Only submit a report if you notice damage so we can fix or replace the copy.
                                    </span>
                                </div>
                            ) : (
                                <div className="space-y-2 max-h-72 overflow-y-auto pr-1 support-scroll">
                                    {myDamageReports.map((r) => (
                                        ...
                                        >
                                            ...
                                            <p className="mt-1 text-[11px] text-white/80">{r.damageType}</p>
                                            {r.notes && (
