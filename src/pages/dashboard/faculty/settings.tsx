    ...
    async function onSubmitPassword(e: React.FormEvent) {
        ...
        try {
            ...
            toast.error("Password change failed", {
                description: String(err?.message || "Failed to change password."),
            })
        } finally {
            setPwBusy(false)
        }
    ...
    async function onResendVerification() {
        ...
        if (!targetEmail || !isValidEmail(targetEmail)) {
            toast.warning("Valid email required", {
                description:
                    "Please make sure your email is saved and valid before sending verification.",
            })
            return
        }
        if (resendCooldown > 0) return
                        ...
                        {user === undefined ? (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                    ...
                                    {facultyId ? (
                                        <div className="rounded-md border border-white/10 bg-slate-900/40 p-3">
                                            <div className="text-xs text-white/60">Faculty ID</div>
                                            <div className="mt-0.5 font-medium">{fmtValue(facultyId)}</div>
                                        </div>
                                    ) : null}
